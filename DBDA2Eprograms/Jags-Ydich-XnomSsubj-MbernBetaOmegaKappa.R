...
source("C:\\Users\\neevs\\OneDrive - Davidson Academy\\Desktop\\10th Grade\\Data Science (OHS)\\DBDA2Eprograms\\DBDA2E-utilities.R")
#===============================================================================
...
genMCMC = function( data , sName="s" , yName="y" ,  
  ...
  # Convert y safely to 0/1 integer
  # (handles logical TRUE/FALSE, factors, etc.)
  y <- as.integer(as.character(yRaw))
  if (any(is.na(y)) && is.logical(yRaw)) y <- as.integer(yRaw)  # TRUE/FALSE -> 1/0
  
  # Convert subject IDs safely to consecutive integers 1..Nsubj
  sFac <- factor(sRaw)          # this is the key step
  s <- as.integer(sFac)
  
  # Optional: remove rows with missing y or s (or stop instead)
  bad <- is.na(y) | is.na(s)
  if (any(bad)) {
    # Either STOP:
    stop(paste0("Found ", sum(bad), " rows with NA in y or s. Fix/remove them before running JAGS."))
    # Or DROP:
    # y <- y[!bad]; s <- s[!bad]
  }
  
  # Validate y values
  if (any(!(y %in% c(0,1)))) stop("All y values must be 0 or 1.")
  
  Ntotal <- length(y)
  Nsubj  <- nlevels(sFac)  # or length(unique(s))
...
plotMCMC = function( codaSamples , data , sName="s" , yName="y" , 
  ...
  # Plot omega, kappa:
  options(repr.plot.width =  3.5*2, repr.plot.height = 3.0)
  par( mfrow=c(1,2) )
  ...
  if ( !is.null(diffIdVec) ) {
    Nidx = length(diffIdVec)
    options(repr.plot.width =  2.5*Nidx, repr.plot.height = 2.0*Nidx)
